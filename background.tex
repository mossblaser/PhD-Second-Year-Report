	...
	\section{Neuromorphic computing}
		...
		\subsection{Analogue and mixed-mode}
			...
			\subsubsection{Neurogrid}
				...
				% XXX: Not leaves in the tree
				% XXX: Supports multicast...
				% XXX: Broadcasts to subtrees requiring them to filter
				% XXX: Uses 1-of-2 code for some comms
				% XXX: Async circuits, only active on events
				...
				% Though tree bandwidth is limited, it avoids deadlock in multicast case
				% due to this structure. Argue that though throughput is root(n) lower,
				% latency is root(n)/log(n) shorter.
				
				% TODO: Interconnection is special purpose, only spikes passed around
				% TODO: Multicast is hard since 65,536 neurons to multicast out to,
				% limits scaling. 12.5 ns per weight lookup, 200 ns to send a packet.
